...
/*==========================Diffuse ==========================*/
                ...
                // return LightMap.g;
                // return lerp(BaseMap*ShadowMap,BaseMap, step(_LightThreshold,NL01 + RampOffsetMask + _RampOffset) * ShadowAOMask);
                // return lerp(BaseMap*ShadowMap,BaseMap, step(_LightThreshold,NL01) );
...
/*==========================Specular ==========================*/
                ...
                //皮革     -裁边视角光
                //金属     -裁边视角光(ViewSpace) 模拟反射 (matcap)
                
                float Layer = LayerMask * 255;
                // return LayerMask;
                // return LayerMask;
                // return LayerMask;
                //边缘光 仅在暗部显示
                float3 N_ViewSpaceS = mul((float3x3)UNITY_MATRIX_V, T);//将平滑后的法线转到 视角空间下
                // return N_ViewSpaceS.x;
                ...
                if(Layer>190)
                {
                    // float3 MetallicStepSpecular = step(1-_MetallicStepSpecularWidth,NL)*_MetallicStepSpecularIntensity*BaseMap; //裁边光源光
                ...
                // return SpecularIntensityMask;
                // return  SpecularIntensity>170;
                ...
                if(Layer>60 && Layer<190)
                {
                   // float SpecularIntensity = pow(SpecularIntensityMask,1/2.2)*255;
                   // float SpecularIntensity = SpecularIntensityMask*255;
                   // float StepSpecularMask = float(SpecularIntensity>0 && SpecularIntensity<128);
