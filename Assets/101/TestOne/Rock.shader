...
Shader "Rock"
{
	Properties
	{
		...
		[ASEEnd][ASEBegin]_InsColor("InsColor", Color) = (0,0,0,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			#endif
			CBUFFER_END
			UNITY_INSTANCING_BUFFER_START(Rock)
				UNITY_DEFINE_INSTANCED_PROP(float4, _InsColor)
			UNITY_INSTANCING_BUFFER_END(Rock)
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 ShadowCoords = float4( 0, 0, 0, 0 );
					...
					#endif
				#endif
				float4 _InsColor_Instance = UNITY_ACCESS_INSTANCED_PROP(Rock,_InsColor);
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = _InsColor_Instance.rgb;
				float Alpha = 1;
